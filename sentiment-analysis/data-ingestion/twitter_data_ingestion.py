def lambda_handler(event, context):
    ...
    class TwitterClient():
        ...
        def get_tweet_based_on_hashtag(self, hashtag_list):
            ...
            for tweet in Cursor(self.api.search, q=hashtag_list).items():
                tweets.append(json.dumps(tweet._json))
                print(json.dumps(tweet._json))
            # print("tweets::", tweets)
            filename = source + '_' + hash_tags + '_' + str(datetime.now()) + '.json'
    ...
    def create_sentiment():
        print("inside create")
        filename = 'stream-data.json'#source + '_' + hash_tags + '_' + str(datetime.now()) + '.txt'
        twitter_client = TwitterClient()
        filename = twitter_client.get_tweet_based_on_hashtag(hash_tags)
  ...
  "Records": [
    {
      ...
      "body": "{\"source\": \"twitter\", \"topic\": \"PrinceFamine\"}",
      "attributes": {
